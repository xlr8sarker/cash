...
async function printMemo(memo, type = "memo") {
  const html = type === "due" ? buildDueReceiptHTML(memo) : buildMemoHTML(memo);
  const printCss = document.querySelector('link[href="print-template.css"]');
  let cssHref = "print-template.css";
  ...
  win.document.write(`
    ...
    <html>
      <head>
        ...
        <style>
          html, body { margin:0; padding:0; background:#fff; display:flex; justify-content:center; }
          @media print {
            @page { size: A4 portrait; margin: 0; }
            body { margin: 0; }
            .memo-page { border-radius:0; border:none; margin:0; box-shadow:none; }
          }
          .memo-page { margin: 20px auto; }
        </style>
      </head>
      <body>${html}</body>
    </html>
