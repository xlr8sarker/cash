...
function wireServiceRow(tr) {
  ...
  const qtyInput = tr.querySelector("td.col-qty .svc-qty");
  const priceInput = tr.querySelector("td.col-price .svc-price");
    ...
    resultsBox.innerHTML = matches
      .map(
        (s) => `<div class="service-search-item" data-name="${escapeHTML(s.name)}" data-price="${s.price}">
          ...
          <span class="svc-search-price">৳${fmt(s.price)}</span>
        </div>`
      )
      .join("");
...
function recalcRowTotal(tr) {
  const qty = parseFloat(tr.querySelector("td.col-qty .svc-qty").value) || 0;
  const price = parseFloat(tr.querySelector("td.col-price .svc-price").value) || 0;
...
function collectServicesFromForm() {
  ...
  rows.forEach((tr) => {
    ...
    const qty = parseFloat(tr.querySelector("td.col-qty .svc-qty").value) || 0;
    const unitPrice = parseFloat(tr.querySelector("td.col-price .svc-price").value) || 0;
    if (name && qty > 0) {
